...
5. **Los movimientos de inventario son inmutables.** Movimientos MP y PT no se editan ni se eliminan; un error se
   corrige con un movimiento de reverso. (PT ya es así; MP no — ver §5.)
...
| --- | --- |
| Admin no puede editar, desactivar ni eliminar a un SuperAdmin | *(nuevo)* |
| Nadie puede desactivarse, eliminarse ni quitarse el rol a sí mismo | `UserController::update/destroy` |
| Siempre debe quedar al menos un SuperAdmin activo | `UserController:133-146` (hoy "último admin") |
