...
| ~~B41~~ | ✅ Hecho (2026-09-22): **auditoría de dependencias.** `composer audit` pasa de 32 avisos en 6 paquetes a 0 (`dompdf` 3.1.6, `phpspreadsheet` 1.30.7 —había uno crítico—, `maatwebsite/excel` 3.1.70, `guzzle` 7.15.5, `psr7` 2.13.1, `commonmark` 2.10.3; todo parches, sin cambios en `composer.json`). `npm audit` pasa de 5 a 0 (`npm audit fix`, dependencias indirectas). Dependabot semanal (Composer y npm) y mensual (GitHub Actions) contra `develop` | Checklist de preproducción | 1,5 h |
