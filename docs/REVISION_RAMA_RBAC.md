...
| ~~B39~~ | ✅ Hecho (2026-09-21): **firmas fuera del disco público.** Se guardan en el disco privado (`User::SIGNATURE_DISK`) y se sirven por `users.signature` con sesión y la policy `UserPolicy::viewSignature` (el propio usuario, `users.edit` o `production_orders.complete`; la condición de `users.edit` aplica la misma regla que `update()`, así que un rol que edita usuarios no ve la firma de quien tiene más permisos). Antes cualquiera con la URL las veía sin iniciar sesión. El certificado de calidad las lee del disco privado (test que lo comprueba) | Checklist de preproducción | 2 h |
