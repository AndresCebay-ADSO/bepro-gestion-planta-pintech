...
| ~~B41~~ | ✅ Hecho (2026-09-22): **auditoría de dependencias.** `composer audit` pasa de 32 avisos en 6 paquetes a 0 (`dompdf` 3.1.6, `phpspreadsheet` 1.30.7 —había uno crítico—, `maatwebsite/excel` 3.1.70, `guzzle` 7.11 → 7.15.5, `psr7` 2.11 → 2.13.1 y `commonmark` 2.8 → 2.10.3 suben de versión menor; el resto, parches). El lock movió 38 paquetes por arrastre, todos dentro de los rangos de `composer.json`, que no cambia salvo `config.platform.php: 8.3` (Composer resuelve siempre para el PHP de Docker, aunque se ejecute con 8.4). `npm audit` pasa de 5 a 0 (`npm audit fix`, dependencias indirectas de compilación y lint). Dependabot semanal (Composer y npm, **incluidas las dependencias indirectas**: 5 de los 6 paquetes vulnerables lo eran) y mensual (GitHub Actions) contra `develop`. Las auditorías del CI pasan a **bloquear** (`composer audit` y `npm audit --omit=dev`; la de desarrollo solo avisa). Requiere en GitHub: rama por defecto `develop` y Dependabot alerts/security updates activados | Checklist de preproducción | 1,5 h |
| ~~B42~~ | ✅ Hecho (2026-09-22): **orden estable en los listados.** Cotizaciones, pedidos, órdenes, usuarios, solicitudes de pintura y auditoría ordenaban solo por fecha; con empates, PostgreSQL devolvía orden arbitrario y la paginación podía repetir o saltarse registros (se manifestó como `QuotationFilterTest` intermitente en CI). Ahora desempatan con `latest('id')`, con `StableListingOrderTest` que falla si se quita (el caso de auditoría solo delata el fallo en PostgreSQL; el CI corre los dos motores). Mismo desempate en los tres sitios que tocan saldos de producción: el FIFO del endpoint (`RemnantConsumptionController::availableRemnants`) y su copia en la ficha de la orden (`BuildProductionOrderShowDataAction`) —donde además decidía qué 50 entraban en el corte y las dos consultas podían discrepar entre sí— y el listado paginado (`RemnantController::index`, que ordenaba con `orderByDesc('created_at')` y por eso no salió en la primera búsqueda de `latest()`). La imagen `workspace` de Docker suma `gd` (la exigía `phpspreadsheet`) | CI 2026-09-22 | 45 min |
...
| C8 | `spatie/laravel-permission` 7 → 8 (mayor, base del RBAC): leer la guía de migración, revisar caché de permisos y guards, y correr la matriz de acceso. Dependabot lo ignora hasta entonces (PR #160) | Dependabot 2026-09-22 |
| C9 | `maatwebsite/excel` 3 → 4 (mayor, cambia la API de exportación): revisar `ProductionOrderExport` y la vista Blade del Excel. Ignorado (PR #159) | Dependabot 2026-09-22 |
| C10 | `@vitejs/plugin-react` 5 → 6 (mayor): retira la opción `babel`, que es la que activa React Compiler; migrar a `@rolldown/plugin-babel` con `reactCompilerPreset` o a la opción `compiler`. Ignorado (PR #165): al fusionarlo, el build pasa pero el compilador queda apagado sin aviso | Dependabot 2026-09-22 |
