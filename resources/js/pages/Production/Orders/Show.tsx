...
export default function ProductionOrderShow({
    ...
    const orderPackagingPlans = useMemo(() => order.packaging_plans ?? [], [order.packaging_plans]);
    const lineAdjustments = useMemo(() => order.line_adjustments ?? [], [order.line_adjustments]);

    const isCompleted = order.status === 'completed';
    const hasOrderData = orderDetails.length > 0 || orderPackagingPlans.length > 0;

    const { data, setData, post, processing, errors } = useForm<ProductionOrderFormData>({
    ...
    const ingredientRows = isCompleted
        ...
        : data.ingredients.map((ingredient) => ({
              ...ingredient,
              unit_cost: previewIngredientsById.get(ingredient.id)?.unit_cost ?? ingredient.unit_cost ?? 0,
              total_cost: previewIngredientsById.get(ingredient.id)?.total_cost ?? ingredient.total_cost ?? 0,
          }));
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div className="space-y-6">
                        ...
                        <OrderInfoCard
                            ...
                            bulkCost={isCompleted ? (order.total_bulk_cost ?? 0) : pendingBulkCost}
                            finishedCost={isCompleted ? (order.total_finished_cost ?? 0) : pendingFinishedCost}
