...
import { create as formulasCreate, show as formulasShow } from '@/routes/formulas';
...
export default function ProductsShow({ product, can }: Props) {
    const handleDelete = () => {
        if (!window.confirm(`¿Eliminar el producto ${product.code}?`)) return;
        ...
        <>
            ...
            <div className="space-y-6 p-6">
                ...
                <div className="grid gap-4 rounded-lg border border-border bg-card p-6 md:grid-cols-3">
                    ...
                    <div>
                        <p className="text-xs uppercase tracking-wide text-muted-foreground">Unidad de Medida</p>
                    ...
                    <div>
                        <p className="text-xs uppercase tracking-wide text-muted-foreground">Precio de Venta</p>
                        <p className="text-sm font-medium text-foreground">
                            {product.current_price ? `$${product.current_price}` : 'No asignado'}
                        </p>
                    </div>
                </div>
                ...
                <div className="rounded-lg border border-border bg-card">
                    <div className="flex items-center justify-between border-b border-border px-6 py-4">
                        <div>
                            <h2 className="font-medium text-foreground">Fórmulas</h2>
                        ...
                        <Button size="sm" asChild>
                            <Link href={formulasCreate({ product_id: product.id }).url}>
                                Nueva Fórmula
                            </Link>
                        </Button>
                    </div>
                    ...
                    <table className="w-full text-sm">
                        <thead className="border-b border-border bg-muted/40">
                            <tr>
                                ...
                                <th className="p-4 text-left font-medium">Creada por</th>
                        ...
                        <tbody>
                            {(product.formulas ?? []).map((formula) => (
                                ...
                                >
                                    <td className="p-4 font-mono font-medium">v{formula.version}</td>
                                    <td className="p-4">
                            ...
                            {(product.formulas ?? []).length === 0 && (
                                <tr>
                                    <td colSpan={6} className="p-8 text-center text-sm text-muted-foreground">
                                        No hay fórmulas registradas. Crea la primera usando el botón "Nueva Fórmula".
                                    </td>
                                </tr>
                            )}
                        </tbody>
