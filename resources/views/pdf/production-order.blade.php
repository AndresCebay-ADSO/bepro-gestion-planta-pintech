...
<body>
    ...
    <table style="margin-top: 8px;">
        ...
        <tr>
            ...
            <td class="value" style="width: 13%;">
                {{ $order['agitation_start_time'] ? \Carbon\Carbon::parse($order['agitation_start_time'])->format('H:i') : '' }}
            </td>
            <td class="label" style="width: 20%;">HORA FINALIZACIÓN AGITACIÓN</td>
            <td class="value" style="width: 13%;">
                {{ $order['agitation_end_time'] ? \Carbon\Carbon::parse($order['agitation_end_time'])->format('H:i') : '' }}
            </td>
        ...
        <tr>
            ...
            <td class="value">
                {{ $order['packaging_start_time'] ? \Carbon\Carbon::parse($order['packaging_start_time'])->format('H:i') : '' }}
            </td>
            <td class="label">HORA FINALIZACIÓN ENVASADO</td>
            <td class="value">
                {{ $order['packaging_end_time'] ? \Carbon\Carbon::parse($order['packaging_end_time'])->format('H:i') : '' }}
            </td>
    ...
    <table style="margin-top: 8px;">
        <tr>
            <td colspan="5" class="section-header">MATERIA PRIMA Y CANTIDADES</td>
        </tr>
        <tr>
        ...
        @foreach($order['details'] as $detail)
            ...
            @endphp
            <tr>
                <td class="text-center">{{ $detail['raw_material']['code'] ?? 'N/A' }}</td>
                <td class="text-center">{{ $kg > 0 ? number_format($kg, 2) : '' }}</td>
                <td class="text-center">{{ $grams > 0 ? number_format($grams, 0) : '' }}</td>
                <td></td>
                <td class="text-center">
                    {{ $detail['actual_quantity'] !== null ? number_format($detail['actual_quantity'], 2) : '' }}
                </td>
            </tr>
        @endforeach
        <tr class="text-bold">
